...
class s3Service:
    ...
    def create_bucket(self, bucket_name):
        try:
            ...
            logger.info("Bucket has been created with name %s", constants.S3_INPUT_BUCKET)
        except Exception as error:
            logger.error("Error occurred while creating the bucket %s", constants.S3_OUTPUT_BUCKET)
            raise error
